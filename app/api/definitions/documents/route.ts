...
export async function GET(request: Request) {
    try {
        ...
        if (!showAll) {
            // @ts-ignore
            query = query.where(eq(documentTypes.isActive, true));
        }

        const data = await query.orderBy(desc(documentTypes.createdAt));
...
export async function POST(request: Request) {
    try {
        ...
        const [newRec] = await db.insert(documentTypes)
            .values({ name })
            .returning();
...
export async function PUT(request: Request) {
    try {
        ...
        const oldData = await db.select().from(documentTypes).where(eq(documentTypes.id, id)).get();
